...
int	sound_init(t_sound *sounds, BOOL *bass)
{
	...
	*bass = BASS_Init(-1, 44100, 0, 0, 0);
	...
	sounds->background = BASS_StreamCreateFile(FALSE, "assets/sounds/back6.mp3", 0, 0,
			BASS_SAMPLE_LOOP);
	sounds->run = BASS_StreamCreateFile(FALSE, "assets/sounds/run/run.mp3", 0, 0, 0);
	sounds->revolver = BASS_StreamCreateFile(FALSE,
			"assets/sounds/weapon/revolver.mp3", 0, 0, 0);
	sounds->shotgun = BASS_StreamCreateFile(FALSE,
			"assets/sounds/weapon/shotgun-frst.mp3", 0, 0, 0);
	sounds->pistol = BASS_StreamCreateFile(FALSE, "assets/sounds/weapon/pistol.mp3",
			0, 0, 0);
	sounds->door_open = BASS_StreamCreateFile(FALSE, "assets/sounds/door/door_3.mp3",
			0, 0, 0);
	sounds->door_close = BASS_StreamCreateFile(FALSE, "assets/sounds/door/door_3.mp3",
			0, 0, 0);
	sounds->revolver_3 = BASS_StreamCreateFile(FALSE,
			"assets/sounds/weapon/science.mp3", 0, 0, 0);
