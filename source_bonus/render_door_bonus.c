...
static void	draw_door_texture_in_draw_door(t_intvector *pos,
	t_data *img, t_master *master, t_draw_door	*draw)
{	
	t_door_data	*data;
	...
	draw->texture_door.x = get_x_coordinate_door_texture(data->door_hitside, master,
		img);
	...
	draw->texture_door.pos = ((draw->start_door - master->view_high)
		- SCREEN_HEIGHT / 2 + data->door_height / 2) * draw->texture_door.step;
	pos->y = draw->start_door;
	while (pos->y < draw->end_door)
	{
		draw->texture_door.y = (int)draw->texture_door.pos % (img->
			door_texture_height[master->type_door][master->index_door] - 1);
		draw->texture_door.pos += draw->texture_door.step;
		draw->texture_door.color = get_door_color(master, &draw->texture_door,
				img);
